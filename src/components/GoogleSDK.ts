...
const fetchGoogleLogin = async (url: string, payload: string, token: string) => {
...
export default class GoogleSDK extends BaseComponent {
  connectedCallback() {
    ...
    if (jsKeyInputEl === null) return;
    if (!(jsKeyInputEl instanceof HTMLInputElement)) return;
    if (serverAuthUrlInputEl === null) return;
    if (!(serverAuthUrlInputEl instanceof HTMLInputElement)) return;
    if (textareaEl === null) return;
    if (!(textareaEl instanceof HTMLTextAreaElement)) return;
    if (googleLoginButtonEl === null) return;
    if (!(googleLoginButtonEl instanceof HTMLButtonElement)) return;

    jsKeyInputEl.placeholder = 'Google JavaScript 키';

    jsKeyInputEl.addEventListener(
      'change',
      debounce(() => {
        const id: string = 'google-sdk';
        const sdk: HTMLScriptElement = document.createElement('script');
