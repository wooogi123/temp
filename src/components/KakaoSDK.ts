...
const fetchKakaoLogin = async (url: string, payload: string, token: string) => {
...
export default class KakaoSDK extends BaseComponent {
  connectedCallback() {
    ...
    if (kakaoLoginButtonEl === null) return;
    if (!(kakaoLoginButtonEl instanceof HTMLButtonElement)) return;
